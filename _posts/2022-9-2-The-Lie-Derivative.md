...
Particularly, recall that the Lie derivative of a basis vector field with respect to another basis vector field is always zero by construction, hence if $\alpha = \text{vol}^n = \sqrt{ g }dx^1 \wedge \dots\wedge dx^n$ is the volume form for a Riemannian manifold $M^n$ and if $X$ is a vector field on the same manifold, then $\mathcal{L}_{X} \text{vol}^n$ is the n-form that reads off the rate of change of volume of a parallelopiped spanned by $n$ vectors that are pushed forward by the flow $\phi_{t}$ generated by $X$. 
