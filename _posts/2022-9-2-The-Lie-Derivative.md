...
$$
that is, 
$$\mathcal{L}_{X}\alpha(Y_{1},\dots,Y_{p})$$ measures the derivative (as one moves along the orbit of $X$) of the value of $\alpha$ evaluated on a p-tuple of vector fields $Y$ that are invariant under the flow  generated by $X$.
...
$$\mathcal{L}_{X} \text{vol}^n$$ is the n-form that reads off the rate of change of volume of a parallelopiped spanned by $n$ vectors that are pushed forward by the flow $\phi_{t}$ generated by $X$. 
