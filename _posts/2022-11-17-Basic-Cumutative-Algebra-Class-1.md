...
- A *prime element* is an element which is not zero or a unit (defined next), satisfying $p \vert ab$ implies that $p \vert a$ or $p|b$, where $p\vert a$ means $p$ divides $a$.
